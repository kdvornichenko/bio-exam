...
export default function ImageComponent({
  ...
  const $onEnter = useCallback(
    (event: KeyboardEvent) => {
      ...
      if (isSelected && $isNodeSelection(latestSelection) && latestSelection.getNodes().length === 1) {
        if (showCaption) {
          // Переместить фокус во вложенный редактор
          $setSelection(null);
          event.preventDefault();
          caption.focus();
          return true;
  ...
  useEffect(() => {
    ...
    const unregister = mergeRegister(
      ...
      editor.registerCommand(
        ...
        (event) => {
          if (event.target === imageRef.current) {
            // TODO Это просто временный обходной путь, чтобы FF вел себя как другие браузеры.
            // В идеале, это должно обрабатывать drag & drop тоже (и все браузеры).
            event.preventDefault();
            return true;
          }
          return false;
  ...
  const onResizeEnd = (nextWidth: "inherit" | number, nextHeight: "inherit" | number) => {
    // Задержка скрытия панелей изменения размера для случая клика
    setTimeout(() => {
      setIsResizing(false);
    }, 200);
