...
// Символы, которые мы ожидаем увидеть в упоминании (не пробел, не пунктуация).
const VALID_CHARS = "[^" + TRIGGERS + PUNC + "\\s]";

// Нестандартная серия символов. Каждая серия должна предшествовать и сопровождаться
// валидным символом.
const VALID_JOINS =
  "(?:" +
  "\\.[ |$]|" + // Например, "r. " в "Mr. Smith"
  " |" + // Например, " " в "Josh Duck"
  "[" +
  PUNC +
  "]|" + // Например, "-' в "Salier-Hellendag"
  ")";
...
// 50 - это лимит длины псевдонима.
const ALIAS_LENGTH_LIMIT = 50;

// Regex, используемый для сопоставления псевдонима.
const AtSignMentionsRegexAliasRegex = new RegExp(
  "(^|\\s|\\()(" + "[" + TRIGGERS + "]" + "((?:" + VALID_CHARS + "){0," + ALIAS_LENGTH_LIMIT + "})" + ")$",
);

// В всплывающем окне отображается не более 5 предложений.
...
function checkForAtSignMentions(text: string, minMatchLength: number): MenuTextMatch | null {
  ...
  if (match !== null) {
    // Стратегия игнорирует начальные пробелы, но нам нужно знать их
    // длину, чтобы добавить ее к leadOffset
