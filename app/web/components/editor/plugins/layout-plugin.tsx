...
export function LayoutPlugin(): null {
  ...
  useEffect(() => {
    ...
    return mergeRegister(
      // Когда макет является последним дочерним элементом, нажатие стрелки вниз/вправо вставит абзац
      // под ним, чтобы можно было добавить больше контента. Это похоже на то, что делает $insertBlockNode
      // (в основном для декораторов), за исключением того, что всегда будет возможность продолжить добавление
      // нового контента, даже если завершающий абзац случайно удален
      ...
      // Когда макет является первым дочерним элементом, нажатие стрелки вверх/влево вставит абзац
      // над ним, чтобы можно было добавить больше контента. Это похоже на то, что делает $insertBlockNode
      // (в основном для декораторов), за исключением того, что всегда будет возможность продолжить добавление
      // нового контента, даже если начальный абзац случайно удален
      ...
      editor.registerCommand(
        ...
        ({ template, nodeKey }) => {
          editor.update(() => {
            ...
            // Добавляем или удаляем лишние колонки, если новый шаблон не совпадает с существующим
      ...
      // Трансформеры, обеспечивающие структуру для каждого типа узла. В случае, если структура вложенности не
      // "Container > Item", он развернет узлы и преобразует их обратно
      // в обычный контент.
