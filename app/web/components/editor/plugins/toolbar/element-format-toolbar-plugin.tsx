...
export function ElementFormatToolbarPlugin({ separator = true }: { separator?: boolean }) {
  ...
  const $updateToolbar = (selection: BaseSelection) => {
    if ($isRangeSelection(selection)) {
      ...
      if ($isLinkNode(parent)) {
        // Если узел является ссылкой, нам нужно получить родительский узел абзаца, чтобы установить формат
  ...
  const handleValueChange = (value: string) => {
    if (!value) return; // Предотвращаем отмену выбора текущего значения
